/*
	Generates a new r type instruction with the given parameters
*/
Instruction* new_r_instruction(unsigned int opcode1, unsigned int s, unsigned int t, unsigned int d, unsigned int shift, unsigned int opcode2) {	
	Instruction* instruction = (Instruction*)malloc(sizeof(Instruction));
	check_alloc((void*)instruction);

	instruction->type = I_TYPE_N;
	instruction->target = NULL;

	R_Data* data = (R_Data*)malloc(sizeof(R_Data));
	check_alloc((void*)data);

	data->opcode1 = opcode1;
	data->s = s;
	data->t = t;
	data->d = d;
	data->shift = shift;
	data->opcode2 = opcode2;
...
/*
	Generates a new i type instruction with the given parameters
*/
Instruction* new_i_instruction(unsigned int opcode, unsigned int s, unsigned int t, unsigned int im, char* target) {
	Instruction* instruction = (Instruction*)malloc(sizeof(Instruction));
	check_alloc((void*)instruction);

	if(target != NULL) instruction->type = I_TYPE_B;
	else instruction->type = I_TYPE_N;
	instruction->target = target;

	I_Data* data = (I_Data*)malloc(sizeof(I_Data));
	check_alloc((void*)data);
...
/*
	Generates a new j type instruction with the given parameters
*/
Instruction* new_j_instruction(unsigned int opcode, char* target) {
	Instruction* instruction = (Instruction*)malloc(sizeof(Instruction));
	check_alloc((void*)instruction);

	instruction->type = I_TYPE_J;
	instruction->target = target;
...
void new_instruction(unsigned int inst, unsigned int rs, unsigned int rt, unsigned int rd, unsigned int im, unsigned int sh, char* tg) {
	switch(inst) {
		...
		case I_NOP:
			...
			break;
		default:
			printf("Unrecognizes instruction\n");
			exit(1);
	}
}